...
$tableName = "tbl_live_language";   

// Search functionality
if (!isset($_GET['keyword'])) {
    $sql_query = "SELECT * FROM tbl_live_language ORDER BY id ASC"; 
} else {
    $keyword = addslashes(trim($_GET['keyword']));
    $sql_query = "SELECT * FROM tbl_live_language WHERE language_name LIKE '%$keyword%' ORDER BY id DESC"; 
}
        ...
        <div class="card h-100">
            <div class="card-top d-md-inline-flex align-items-center justify-content-between py-3 px-4">
                ...
                <div class="d-flex mt-2 mt-md-0">
                    ...
                    <a href="create_language.php?add=yes" class="btn btn-primary d-inline-flex align-items-center justify-content-center">
                        <i class="ri-add-line"></i>
                        <span class="ps-1 text-nowrap d-none d-sm-block">Create Language</span>
                    </a>
                </div>
            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="nsofts-switch d-flex align-items-center enable_disable" data-bs-toggle="tooltip" data-bs-placement="top" title="Enable / Disable">
                                                <input type="checkbox" id="enable_disable_check_<?= $i ?>" data-id="<?= $row['id'] ?>" data-table="<?=$tableName ?>" data-column="language_status" class="cbx hidden btn_enable_disable" <?php if ($row['language_status'] == 1) { echo 'checked'; } ?>>
                                                <label for="enable_disable_check_<?= $i ?>" class="nsofts-switch__label"></label>
                                            </div>
                                        </td>
                                        <td>
                                            <a href="create_language.php?id=<?php echo $row['id']; ?>" class="btn btn-primary btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
